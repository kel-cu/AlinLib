...
public class SliderConfigPercent extends SliderPercent implements Resetable {
    ...
    public SliderConfigPercent(int x, int y, Config config, String typeConfig, int defaultConfig, Component component) {
        this(x, y, DEFAULT_WIDTH(), DEFAULT_HEIGHT, GuiUtils.getSelected(), config, typeConfig, defaultConfig, component);
    }
    public SliderConfigPercent(int x, int y, AbstractStyle style, Config config, String typeConfig, double defaultConfig, Component component) {
        this(x, y, DEFAULT_WIDTH(), DEFAULT_HEIGHT, style, config, typeConfig, defaultConfig, component);
    }
    ///
    public SliderConfigPercent(int x, int y, int width, int height, Config config, String typeConfig, int defaultConfig, Component component) {
        this(x, y, width, height, GuiUtils.getSelected(), config, typeConfig, defaultConfig, component);
    }
    public SliderConfigPercent(int x, int y, int width, int height, AbstractStyle style, Config config, String typeConfig, double defaultConfig, Component component) {
        super(x, y, width, height, style, config.getNumber(typeConfig, defaultConfig).doubleValue(), component);
        this.config = config;
